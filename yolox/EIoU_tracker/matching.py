...
from yolox.EIoU_tracker import kalman_filter
...
def embedding_distance_all(tracks, detections, metric='cosine'):
    """
    :param tracks: list[STrack]
    :param detections: list[BaseTrack]
    :param metric:
    ...
    if cost_matrix.size == 0:
        return cost_matrix
    det_features = np.asarray([track.curr_feat for track in detections], dtype=np.float64)
    # print("det_features:", det_features.shape)
    # track_features = np.asarray([track.smooth_feat for track in tracks], dtype=np.float)
    # print("track_features:", track_features.shape)

    # my change
    track_features = np.asarray([np.mean(track.features, axis=0) for track in tracks], dtype=np.float64)
    # print("mean_track_features:", mean_track_features.shape)
